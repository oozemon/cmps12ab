...
inode_ptr selected_file =
  state.get_cwd()->to_get_dir(words.at(i));
 if(selected_file->get_type() != 0)
   throw yshell_exn(words.at(0)+": "+
    words.at(1)+ ": "+ "is not a file");
...
inode_ptr selected_dir = state.get_cwd()->to_get_dir(new_dir);
 if(selected_dir->get_type() != 1)
   throw yshell_exn(words.at(0)+": "+words.at(1)+ ": "+ "is not a directory");
...
void fn_lsr (inode_state& state, const wordvec& words){
  ...
  cout << ":" << endl;
  state.get_cwd()->recursive_print_contents(_path);
   DEBUGF ('c', state);
   DEBUGF ('c', words);
}
...
void fn_prompt (inode_state& state, const wordvec& words){
   ...
   DEBUGF ('c', words);
   string error{};
   error += words.at(0) + ": " + "No specified file name";
   if (words.size() < 2) {
     throw yshell_exn(error);
   }
     state.op_prompt(words);
}
...
void fn_rm (inode_state& state, const wordvec& words){
  inode_ptr i = state.get_cwd()->to_get_dir(words.at(1));
  ...
  state.get_cwd()->to_rm_dir(words[1]);
   DEBUGF ('c', state);
   DEBUGF ('c', words);
}
...
void fn_rmr (inode_state& state, const wordvec& words){
  ...
  inode_ptr i = state.get_cwd()->to_get_dir(words[1]);
